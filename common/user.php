...
class User{
    ...
    function stm_attendance_detail($userID, $totalCL, $totalSL, $totalAB,$totalHL,$totalSpend,$totalRemaining,$year){
    
        $query = $this->conn->prepare("INSERT INTO `stm_attendance_user_details` (`userID`, `totalCL`, `totalSL`, `totalAB`,`totalHL`,`totalSpend`,`totalRemaining`,`year`) VALUES ('$userID', '$totalCL', '$totalSL', '$totalAB','$totalHL','$totalSpend','$totalRemaining','$year')");
        
        $ex = $query->execute();
        return $query;
    }
    function stm_attendance($userID,$present,$leave,$attendanceDate){
    
        $query = $this->conn->prepare("INSERT INTO `stm_attendance` (`userID`, `present`, `leave`, `attendanceDate`) VALUES ('$userID', '$present', '$leave', '$attendanceDate')");
        $ex = $query->execute();
        return $query;
    }
