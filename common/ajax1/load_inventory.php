    ...
    $output .= '<tr>';
        ...
        if($res){
          foreach($res as $data){  
            ...
            $amzosData = $db_helper->SingleDataWhere('stm_listing',"LwSku = '".$data['LwSku']."' AND ChannelID = '1' AND StoreID = '5'");
              
            if($amzosData == false){
              $amzOsBack = "style=background-color:#ffe6e6;";
            }else{
              if($data['AmzOsVal'] == true){
            ...
            $AmzQcVal = "";
            $AmzQcBack = "";
            $amzqcData = $db_helper->SingleDataWhere('stm_listing',"LwSku = '".$data['LwSku']."' AND ChannelID = '1' AND StoreID = '6'"); 
            if($amzqcData == false){
              $AmzQcBack = "style=background-color:#ffe6e6;";
            }else{
              if($data['AmzQcVal'] == true){
            ...
            $EbayOsVal = "";
            $EbayOsBack = "";
            $ebyosData = $db_helper->SingleDataWhere('stm_listing',"LwSku = '".$data['LwSku']."' AND ChannelID = '2' AND StoreID = '1'");
            
            if($ebyosData == false){
              $EbayOsBack = "style=background-color:#ffe6e6;";
            }else{
              if($data['EbayOsVal'] == true){
            ...
            $EbayAoBack = "";
              ...
              $ebayAo = $db_helper->SingleDataWhere('stm_listing',"LwSku = '".$row['LWSKU']."' AND ChannelID = '2' AND StoreID = '2'");
              ...
              $output .= '<td align="right"'.$amzOsBack.'></td>';
              $output .= '<td align="right"'.$amzOsBack.'></td>';

              $output .= '<td align="right"'.$AmzQcBack.'></td>';
              $output .= '<td align="right"'.$AmzQcBack.'></td>';

              $output .= '<td align="right"'.$EbayOsBack.'></td>';
              $output .= '<td align="right"'.$EbayOsBack.'></td>';

              $output .= '<td align="right"'.$EbayAoBack.'></td>';
              $output .= '<td align="right"'.$EbayAoBack.'></td>';
