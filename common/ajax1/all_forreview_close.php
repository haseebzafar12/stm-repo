...
$dataStatusDone = $db_helper->SingleDataWhere('stm_statuses','statusName = "Approved"');

$query = "SELECT DISTINCT taskID FROM stm_taskassigned WHERE taskSupervisorID = '$session_id' AND taskStatusID = '".$dataStatusDone['id']."' ";
...
if($total_data > 0)
{
  foreach($result as $row_assignee)
  {
    ...
    if($datawheproio['taskpriorityName'] == "4-Normal"){
      ...
      $slect = "";
      if($total_approved == $total_record_supervisor){
        $status = "stm_statuses";
    ...
    $taskName = strip_tags($row['taskName']);
      ...
      if (strlen($taskName) > 30) {
          // truncate string
          $stringCut = substr($taskName, 0, 30);
    ...
    <tr>
      ...
      <td>'.$supplier.'</td>
      <td>'.$row['taskName'].'</td>
      <td class="selected_'.$row['id'].'">'.$slect.'</td>
      <td>
