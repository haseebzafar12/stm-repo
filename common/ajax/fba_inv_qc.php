...
$session_id = $_SESSION['id'];
    ...
    $output = "";
    if(!empty($_FILES) AND !empty($_POST['FBA_amz_qc'])){
        ...
        $filename=$_FILES["fileName"]["tmp_name"];
         ...
         if($_FILES["fileName"]["size"] > 0)
         {
            ...
            $st = $db->prepare('truncate table stm_fba_qc');
            $st->execute();
            while (($getData =fgetcsv($file, ",")) !== FALSE)
            {
                ...
                if (in_array($row, $skip_row_number))   
                {
                    ...
                    if($getData[4] == "SELLABLE"){
                      $statement = $db->prepare("INSERT into stm_fba_qc (StoreSKU,StoreItemID,Stock,LwSku) values('$storeSKU','$StoreItemID','$Stock',NULL)");
            ...
            $que = $db->prepare("INSERT IGNORE INTO stm_linked_item (StoreItemID,LwSku) SELECT StoreItemID,LwSku FROM stm_fba_qc");
            $que->execute();

            $stm = $db->prepare("UPDATE stm_fba_qc INNER JOIN stm_linked_item ON stm_linked_item.StoreItemID = stm_fba_qc.StoreItemID SET stm_fba_qc.LwSku = stm_linked_item.LwSku WHERE stm_fba_qc.StoreItemID = stm_linked_item.StoreItemID ");
            $stm->execute();

            $query = $db->prepare("INSERT IGNORE into stm_listing (ItemMasterID,LwSku,ChannelID,StoreID,StoreItemID,StoreSKU,StockType) select 0 as ItemMasterID,LwSku,1 as ChannelID,6 as StoreID,StoreItemID,StoreSKU,'FBA' as StockType from stm_fba_qc");
            $query->execute();

            $date = date('Y-m-d',strtotime($_POST['FBA_amz_qc']));

            $query = $db->prepare("INSERT IGNORE into stm_itemmaster(LWSKU,ItemName,ItemBarCode,SupplierID,FBAStockOs,FBAStockQc,FBMStock,TotalStock,AmzOsSalesQty,AmzQcSalesQty,EbayOsSalesQty,EbayAcSalesQty,AmzOsSalesVal,AmzQcSalesVal,EbayOsSalesVal,EbayAcSalesVal,ItemStatus,AmzOsListed,AmzQcListed,EbayOsListed,EbayAoListed,WebOsListed,WebAcListed,ToListAmzOs,ToListAmzQc,ToListEbayOs,ToListEbayAo,ToListWebOs,ToListWebAc) select DISTINCT LwSku,null as ItemName,null as ItemBarCode,0 as SupplierID,0 as FBAStockOs,0 as FBAStockQc,0 as FBMStock,0 as TotalStock,null as AmzOsSalesQty,null as AmzQcSalesQty,null as EbayOsSalesQty,null as EbayAcSalesQty,null as AmzOsSalesVal,null as AmzQcSalesVal,null as EbayOsSalesVal,null as EbayAcSalesVal,1 as ItemStatus,0 as AmzOsListed,0 as AmzQcListed,0 as EbayOsListed,0 as EbayAoListed,0 as WebOsListed,0 as WebAcListed,null as ToListAmzOs,null as ToListAmzQc,null as ToListEbayOs,null as ToListEbayAo,null as ToListWebOs,null as ToListWebAc from stm_fba_qc");
            $query->execute();

            $dbs = $db->prepare("SELECT LwSku,SUM(stock) as totalStock FROM stm_fba_qc WHERE LwSku != '' GROUP BY LwSku");
            $dbs->execute();
            ...
            foreach($result as $row){
                ...
                $lwskus = $row['LwSku'];
                $it = $db->prepare("UPDATE stm_itemmaster set FBAStockQc = '$fbastock',TotalStock= (FBAStockOs+FBAStockQc+FBMStock) WHERE LWSKU = '$lwskus'");
                $it->execute();
            }
