$(document).ready(function(){
  ...
  $(document).on('click', '.userStatus', function(){
    ...
    if(userstatus == 'inactive'){
      var post_m  = "inactive-user";
      var msg = 'Are you sure you want to in-active?';
      var endMsg = 'User In-Active!';
    }else if(userstatus == 'active'){
      var post_m  = "active-user";
      var msg = 'Are you sure you want to active?';
      var endMsg = 'User Activated!';
    }
    swal({
      title: msg,
      type: 'warning',
      ...
      if (result.value) {
        $.ajax({
            ...
            dataType: 'text',
            data:{userID:userID,post_m:post_m},
  ...
  $(document).on('click', '.saveAttend', function(){
    ...
    var post_m = "attend";
    if(attenDate > currentDate){
      $("#alert_error").text("Please select current date or lesser it");
    }else{
      $.ajax({
          method: "post",
          url:"common/ajax/ajax.php",
          dataType: 'text',
          data:{userID:userID,attenDate:attenDate,currentDate:currentDate,leave:leave,attend:attend,post_m:post_m},
          success:function(data){
            if(data == "error"){
              $("#alert_error").text("");
              $("#alert_error").text("Duplicate Entry, You already marked your attendance to this date");
            }else{
              swal({
...
$(document).ready(function(){
  $(document).on('change', '.multiUpdate', function(){
      
    var stockStatus = $(this).val();
    if(stockStatus != ''){
      var price = [];
    
      var checkedValues = $('input[name="update[]"]:checked').val();
      if(checkedValues > 0){
      ...
      var masterids = $(".master_ids").val(price);
      var ids = $(".master_ids").val();
      
      if(stockStatus == 'discontinue'){
        var post_m  = "discontinue_mul";
        var msg = 'Are you sure you want to discontinue?';
        var endMsg = 'Items Discontinued!';
      }else if(stockStatus == 'continue'){
        var post_m  = "continue_mul";
        var msg = 'Are you sure you want to continue?';
        var endMsg = 'Items Continued!';
      }
...
$(document).ready(function(){
  $(document).on('click', '.listingUp', function(){
    
    var masterIDs= $(".master_id").val();
    ...
    var post_m = "listSubmit";
    
    $.ajax({
        method: "post",
        url:"common/ajax/ajax.php",
        dataType: 'text',
        data:{masterIDs:masterIDs,amzoschecked:amzoschecked,amzqcchecked:amzqcchecked,ebayoschecked:ebayoschecked,ebayaochecked:ebayaochecked,post_m:post_m},
        success:function(data){
          location.reload(true);
        }
    });
  ...
  $(document).on('click', '.channelSubmit', function(){
    ...
    $.ajax({
        ...
        success:function(data){
          ...
          $("#DNlistModal").modal('hide');
          
          data = $.parseJSON(data);
          
          if(data['amzos_output'] == 'null'){
            $(".AmzOsTd_"+masterID).html("");
            $(".AmzOsTd_"+masterID).css('background-color','#b3e6cc');
              
          }else if(data['amzos_output'] == 'zero'){
            $(".AmzOsTd_"+masterID).html("");
            $(".AmzOsTd_"+masterID).css('background-color','#ffcccc');
          }else if(data['amzos_output'] == 'green'){
            $(".AmzOsTd_"+masterID).css('background-color','#b3e6cc'); 
          }else{
            $(".AmzOsTd_"+masterID).html(data['amzos_output']);
            $(".AmzOsTd_"+masterID).css('background-color','white');
          }

          if(data['amzqc_output'] == 'null'){
            $(".AmzQcTd_"+masterID).html("");
            $(".AmzQcTd_"+masterID).css('background-color','#b3e6cc');
              
          }else if(data['amzqc_output'] == 'zero'){
            $(".AmzQcTd_"+masterID).html("");
            $(".AmzQcTd_"+masterID).css('background-color','#ffcccc');
          }else if(data['amzqc_output'] == 'green'){
            $(".AmzQcTd_"+masterID).css('background-color','#b3e6cc'); 
          }else{
            $(".AmzQcTd_"+masterID).html(data['amzqc_output']);
            $(".AmzQcTd_"+masterID).css('background-color','white');
          }

          if(data['ebayos_output'] == 'null'){
            $(".EbayOsTd_"+masterID).html("");
            $(".EbayOsTd_"+masterID).css('background-color','#b3e6cc');
              
          }else if(data['ebayos_output'] == 'zero'){
            $(".EbayOsTd_"+masterID).html("");
            $(".EbayOsTd_"+masterID).css('background-color','#ffcccc');
          }else if(data['ebayos_output'] == 'green'){
            $(".EbayOsTd_"+masterID).css('background-color','#b3e6cc'); 
          }else{
            $(".EbayOsTd_"+masterID).html(data['ebayos_output']);
            $(".EbayOsTd_"+masterID).css('background-color','white');
          }

          if(data['ebayao_output'] == 'null'){
            $(".EbayAoTd_"+masterID).html("");
            $(".EbayAoTd_"+masterID).css('background-color','#b3e6cc');
              
          }else if(data['ebayao_output'] == 'zero'){
            $(".EbayAoTd_"+masterID).html("");
            $(".EbayAoTd_"+masterID).css('background-color','#ffcccc');
          }else if(data['ebayao_output'] == 'green'){
            $(".EbayAoTd_"+masterID).css('background-color','#b3e6cc'); 
          }else{
            $(".EbayAoTd_"+masterID).html(data['ebayao_output']);
            $(".EbayAoTd_"+masterID).css('background-color','white');
          }
...
$(document).ready(function() {
  ...
  $(document).on('change', '.filterByList', function(){
    var filterVal = $(this).val();
    if(filterVal == "supplier"){
      $(".supp_listing").css("display","block");
      $(".stockRes").css("display","none");
      $('.super_list').css('display','none');
    }else if(filterVal == "stock"){
      $(".stockRes").css("display","block");
      $(".supp_listing").css("display","none");
      $('.super_list').css('display','none');
    }else if(filterVal == "supervisor"){
      $('.super_list').css('display','block');
      $(".stockRes").css("display","none");
      $(".supp_listing").css("display","none");
    }
  });
  $(document).on('change', '.filterByStock', function(){
    var filterVal = $(this).val();
    if(filterVal == "supplier"){
      $(".suppliers").css("display","block");
      $(".stock_res").css("display","none");
      $('.supervisors').css('display','none');
    }else if(filterVal == "stock"){
      $(".stock_res").css("display","block");
      $(".suppliers").css("display","none");
      $('.supervisors').css('display','none');
    }else if(filterVal == "supervisor"){
      $('.supervisors').css('display','block');
      $(".stock_res").css("display","none");
      $(".suppliers").css("display","none");
    }
  });  
...
$(document).ready(function() {
  ...
  $(document).on('click', '.uploadCSV', function(){
    ...
    if( document.getElementById("UploadCSV").value.toLowerCase().lastIndexOf(".csv")==-1){
  ...
  $(document).on('click', '.importFbaInvOs', function(){ 
    if( document.getElementById("importFbaInvOs").value.toLowerCase().lastIndexOf(".csv")==-1){
      alert('Please upload a file with .csv extension.');
    }else{ 
  ...
  $(document).on('click', '.importFbaInvQc', function(){ 
    ...
    if( document.getElementById("importFbaInvQc").value.toLowerCase().lastIndexOf(".csv")==-1){
      alert('Please upload a file with .csv extension.');
    }else{
  ...
  $(document).on('click', '.import_amz_os', function(){
    if( document.getElementById("AmzOSFile").value.toLowerCase().lastIndexOf(".csv")==-1){
      alert('Please upload a file with .csv extension.');
    }else{
  ...
  $(document).on('click', '.import_amz_qc', function(){
    if( document.getElementById("AmzQCFile").value.toLowerCase().lastIndexOf(".csv")==-1){
      alert('Please upload a file with .csv extension.');
    }else{
  ...
  $(document).on('click', '.import_ebay_os', function(){
    if( document.getElementById("EbayOSFile").value.toLowerCase().lastIndexOf(".csv")==-1){
      alert('Please upload a file with .csv extension.');
    }else{
  ...
  $(document).on('click', '.import_ebay_ac', function(){
    if( document.getElementById("EbayACFile").value.toLowerCase().lastIndexOf(".csv")==-1){
      alert('Please upload a file with .csv extension.');
    }else{
...
$(document).ready(function(){
  ...
  $(document).ready(function(){
    ...
    $(document).on('click', '#saveContent', function(){
      // Assuming you are using Quill, uncomment the following lines if needed
      // var lContent = quill.root.innerHTML;
      // var listingContent = lContent.replace(/'/g, "\\'");
      
      // If you are using another method to get listingContent, make sure it is defined.
  
      var taskID = $(".taskID").val();
      var editor = CKEDITOR.instances.editor1;
      var listingContent = editor.getData();
      console.log(content);
      ...
      $.ajax({
          ...
          success: function (data) {
              swal({
                  title: 'CONTENT SAVED!',
                  icon: 'success', // 'type' is deprecated, use 'icon' instead
                  buttons: false,
...
$(document).ready(function(){
          ...
          $(document).on('keyup', '#issueNote', function(){
           ...
           $(document).on('click', '.updateBrand', function(){
            ...
            $(document).on('click', '.updateSupplier', function(){
                ...
                data:{id:id,post_m:post_m},
                    success:function(data){
                        ...
                        $('.supervisorTypeSP').text(data['supervisor']);
            ...
            $(document).on('click', '.save_supplier', function(){
                ...
                url:"common/ajax/ajax_data.php",
                dataType: 'text',
                data:{supervisorID:supervisorID,supplierID:supplierID,supplierName:supplierName,supplierType:supplierType,supplierTypeID:supplierTypeID,post_m:post_m},
                success:function(data){
    ...
    load_data_stores(1);
        ...
        function load_data_stores(page, query = '')
        {
          $.ajax({
            url:"common/ajax/stm_stores.php",
            method:"POST",
            data:{page:page, query:query},
            success:function(data)
            {
              $('.storesTable').html(data);
            }
          });
        }
        ...
        $(document).on('click', '.page-link', function(){
          ...
          var query = $('#stores_search_box').val();
          load_data_stores(page, query);
        });

        $('#stores_search_box').keyup(function(){
          var query = $('#stores_search_box').val();
          load_data_stores(1, query);
        });
...
$(document).ready(function(){
  ...
  function load_channel_listing(page, query = '', supplier = '',stock = '',supervisor='')
  {
    var supplier = $('.supp_listing').val();
    $.ajax({
      url:"common/ajax/channelListingajax.php",
      method:"POST",
      data:{page:page, query:query, supplier:supplier, stock:stock, supervisor:supervisor},
      beforeSend: function(){
        var html = "";        
  ...
  $(document).on('click', '.page-link', function(){
    ...
    var supplier = $(".supp_listing").val();
    var stock = $(".stockRes").val();
    load_channel_listing(page,query,supplier,stock);
  });

  $(document).on('change', '.supp_listing', function(){
    var supplier = $(this).val();
    if(supplier != ''){
      $("#exportall").css('display','none');
      $("#exportDiv").html('<a href="listingexport.php?supplierid='+supplier+'" class="btn btn-danger btn-sm">Export</a>');
    }else{
      $("#exportDiv").html('<a href="listingexport.php" class="btn btn-danger btn-sm" id="exportall">Export</a>');
    }   
    load_channel_listing(1,query = '',supplier,stock='');
  });

  $(document).on('change', '.stockRes', function(){
    var stock = $(this).val();
    load_channel_listing(1,query = '',supplier='',stock,supervisor='');
  });

  $(document).on('change', '.super_list', function(){
...
$(document).ready(function(){
  load_inventory1(1);
  function load_inventory1(page, supplier = '', inv='', stock='',supervisor='')
  {
    $.ajax({
      url:"common/ajax/load_inventory.php",
      method:"POST",
      dataType:"text",
      data:{page:page, supplier:supplier, inv:inv, stock:stock,supervisor:supervisor},
      beforeSend: function(){
        var html = "";        
        html += '<div id="imageloading" style="margin-left:34%; margin-top:5%;"><img src="images/loading-icon.gif" height="150" width="150"><br><h4 style="margin-left:7%;">Loading...</h3></div>';
        $(".inventory").html(html);
  ...
  $(document).on('click', '.page-link', function(){
    ...
    var inv = $("#inv").val();
    var stock = $('.stock_res').val();
    var supervisor = $('.supervisors').val();
    load_inventory1(page,supplier,inv,stock,supervisor);
  });
  ...
  $(document).on('change', '.suppliers', function(){
    ...
    if(supplier != ''){
      $("#exportall").css('display','none');
      $("#exportDiv").html('<a href="saletrendexport.php?supplierid='+supplier+'" class="btn btn-danger btn-sm">Export</a>');
    }else{
      $("#exportDiv").html('<a href="saletrendexport.php" class="btn btn-danger btn-sm" id="exportall">Export</a>');
    }    
    load_inventory1(1,supplier,inv = '',stock='',supervisor='');
  });

  $(document).on('change', '.stock_res', function(){
    var stock = $(this).val();
    load_inventory1(1,supplier='',inv = '',stock,supervisor='');
  });

  $(document).on('change', '.supervisors', function(){
    var supervisor = $(this).val();
    load_inventory1(1,supplier='',inv = '',stock='',supervisor);
  });
